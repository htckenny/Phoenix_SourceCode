...
int setbeacon(){
	 ...
	 unsigned int reg=20;  // set AX25 beacon
	  uint8_t txdata[20];
	  txdata[0] = reg;
	  txdata[1] = 15;  // repeat time
	  txdata[2] = 0;   // repeat time
				 	 txdata[3] = 78;//NCKU-
				 	 txdata[4] = 67;
				 	 txdata[5] = 75;
				 	 txdata[6] = 85;
				 	  ...
				 	  txdata[13] = 73;
				 	  txdata[14] = 88;
	  if ( i2c_master_transaction(0,81, txdata,15,0,0,0) == E_NO_ERR) {
		  return 1;
	  }
	  else{
...
int delete_buf(){
	 ...
	 unsigned int reg=36;  // delete a frame in receive buffer   // no resp
	  ...
	  if ( i2c_master_transaction(0,80, txdata,1,0,0,0) == E_NO_ERR) {
...
int arm_ant(){
	  ...
	  if ( i2c_master_transaction(0,49,txdata,1,0,0,0) == E_NO_ERR) {
	        return 1;
	  }
	  else
	  return 0;
...
int callsign(){
    ...
    uint8_t txdata2[8]; // FROM buffer size
	  ...
	  if ( i2c_master_transaction(0,81, txdata,8,0,0,0) == E_NO_ERR) {
		  if ( i2c_master_transaction(0,81, txdata2,8,0,0,0) == E_NO_ERR)
	        return 1;
		  else
			  return 0;
	  }
...
int deploy_ant(){
	  ...
	  txdata[0] = reg;
	  txdata[1] = para;
	  if ( i2c_master_transaction(0,49, txdata,2,0,0,0) == E_NO_ERR) {
	        return 1;
	  }
	  else
	  return 0;
...
int tx_mode(int mode){
     ...
     if(para==1)
	  ...
	  txdata[0] = reg;
	  txdata[1] = para;
	  if ( i2c_master_transaction(0,81,txdata,2,0,0,0) == E_NO_ERR) {
	        return 1;
	  }
	  else
	  return 0;
...
int return_error(){
	...
	txdata[0]=16;
			    ...
			    if ( i2c_master_transaction(0,81,txdata,6,0,0,0) == E_NO_ERR) {
			    				  return 1;}
			    else  return 0;
...
int rccs(){ //Request COM Current State
	     ...
	     uint8_t val[rx_length];  // have 16 byte response
		  ...
		  if ( i2c_master_transaction(0,80, txdata,1,&val,rx_length,7) == E_NO_ERR) {
			  txdata[0]=16;
			  for(int a=1;a<(rx_length+1);a++)
				  txdata[a]=val[a-1];

			  //    vTaskDelay(100);
		    if ( i2c_master_transaction(0,81,txdata,17,0,0,0) == E_NO_ERR) {
		    				  return 1;}
			  else{
...
int recs(){  //Request EPS Current State
	     ...
	     uint8_t val[rx_length];  // have 16 byte response
		  ...
		  if ( i2c_master_transaction(0,2, txdata,1,&val,rx_length,7) == E_NO_ERR) {
			  txdata[0]=16;
		    ...
		    if ( i2c_master_transaction(0,81,txdata,44,0,0,0) == E_NO_ERR) {
		    				  return 1;}
			  else{
...
int execute(char * txbuf){   // send back received command to GS
	...
	printf("Data Type: %d Day: %d  Serial number: %d \r\n",datatype,txbuf[3],ssn);
      ...
      if(down_data_read(txbuf[3],datatype,a,101,&dframe)==1){
    	  ...
    	  txdata[0]=16;
    	  if ( i2c_master_transaction(0,81,txdata,102,0,0,0) != E_NO_ERR) {
    				  return 1;}
    	  else
    				  return 0;
      }
	...
	case 0x21:     //test downlink a-->z
		printf("Command Type: 0x21  downlink a-->z \r\n");
			  ...
			  if ( i2c_master_transaction(0,81, txdata,27,0,0,0) == E_NO_ERR) {
			        return 1;
			  }
			  else
			  return 0;
	...
	case 0x22:  //test downlink 0~236
		...
		printf("Command Type: 0x21  downlink 0~236 \r\n");
		     ...
		     for(int n=1;n<238;n++){
			  ...
			  if ( i2c_master_transaction(0,81, txdata,236,0,0,0) == E_NO_ERR) {
			        return 1;
			  }
			  else
			  return 0;
	...
	case 0x35:   // SEUV_ON
	case 0x36:   // SEUV_OFF
	case 0x38:   //Shutdown transmitter antenna
		printf("Command Type: 0x38  Shutdown transmitter antenna \r\n");
		shutdown_flag_set(1);

		 if(tx_mode(0)==1)
		return 1;
		 else{
			 printf("Shutdown transmitter antenna fail \r\n");
			 return 0;
		    }


	case 0x39:   //Resume transmitter antenna
		printf("Command Type: 0x39  Resume transmitter antenna \r\n");
		shutdown_flag_set(0);
		 if(tx_mode(1)==1){
	...
	case 0x52://Request COM Current State
		printf("Command Type: 0x52  Request COM Current State  \r\n");
		if(rccs()==1)
		return 1;
		else
		return 0;
	case 0x53://Request EPS Current State
		printf("Command Type: 0x53  Request EPS Current State \r\n");
				if(recs()==1)
				return 1;
				else
				return 0;
...
int send_back(void * txbuf){   // send back received command to GS
	  ...
	  if ( i2c_master_transaction(0,81, txdata,18,0,0,0) == E_NO_ERR) {
	        return 1;
	  }
	  else
	  return 0;
...
int send_test(){   // send back received command to GS
	 ...
	 unsigned int reg=17;
	  ...
	  if ( i2c_master_transaction(0,81, txdata,240,0,0,0) == E_NO_ERR) {
	        return 1;
	  }
	  else
	  return 0;
...
void vTaskInitBooting(void * pvParameters) {
	...
	*/
	...
	if(shutflag==1){
		 printf("Shutdown Command Detected!! \r\n");
		  while(0){
			 if(tx_mode(0)==1){
				 printf("tx_mode set not to send!! \r\n");
	...
	break;}
		...
		break;}
			 ...
			 printf("callsign set fail \r\n");
		}}


	//extern void vTimer(void * pvParameters);
	//	xTaskCreate(vTimer, (const signed char *) "TIME", 1024*4, NULL, 2, NULL);
