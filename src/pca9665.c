...
static int pca9665_init_registers(int handle) {
	...
	/* Setup: I2C mode */
	pca9665_write_reg(handle, I2CMODE, 0x01);	/* 0x00 for byte mode, 0x01 for buffered mode */

	/* Setup: I2C Timeout */
	pca9665_write_reg(handle, I2CTO, 0xFF);  //0xFF
	...
	pca9665_write_reg(handle, I2CSCLL, (uint8_t) (period * 0.66));  //0.66
	pca9665_write_reg(handle, I2CSCLH, (uint8_t) (period * 0.33));  //0.33
	...
	for (handle = 0; handle < pca9665_device_count; handle++) {
		...
		case STA_M_DATA_SENDT_ACKED:
			...
			if (len > 0) {
				...
				if (device[handle].rx.frame->dest == 0x02)	/* for GomSpace NanoPower EPS use*/
					pca9665_write_reg(handle, I2CCON, CON_ENSIO | CON_MODE | CON_AA | CON_STA);
				else /* for other slave device, should test more to ensure */
					pca9665_write_reg(handle, I2CCON, CON_ENSIO | CON_MODE | CON_AA | CON_STA | CON_STO);

				// pca9665_write_reg(handle, I2CCON, 0xf7);
				break;
...
int i2c_send(int handle, i2c_frame_t * frame, uint16_t timeout) {
	...
	if (device[handle].tx.queue == NULL) {
		device[handle].tx.queue = xQueueCreate(8, sizeof(i2c_frame_t *));
	}
	// printf("1 I2CSTA\t\t: 0x%02X\n\r", pca9665_read_reg(0, I2CSTA));
...
int i2c_receive(int handle, i2c_frame_t ** frame, uint16_t timeout) {
	...
	if ((device[handle].rx.queue == NULL)) {
		device[handle].rx.queue = xQueueCreate(8, sizeof(i2c_frame_t *));
	}

	if (device[handle].rx.queue == NULL) {
		return E_NO_DEVICE;
	}
	// printf("4 I2CSTA\t\t: 0x%02X\n\r", pca9665_read_reg(0, I2CSTA));
...
int i2c_master_transaction(int handle, uint8_t addr, void * txbuf, size_t txlen, void * rxbuf, size_t rxlen, uint16_t timeout) {
	//printf("enter i2c master transaction\n");
	//pca9665_dump_regs( handle);
	if (handle >= pca9665_device_count)
		return E_NO_DEVICE;
	...
	if (rxlen == 0) {
		/* If the slave node is ADCS(0d87), send additional nonsense command */
		// if (addr == 87) {
		// 	frame->dest =  127;
		// 	i2c_send(handle, frame, 0) ;
		// }
		/*-----------------------*/
		device[handle].callback = tmp_callback;
		xSemaphoreGive(i2c_lock);
	...
	// if (addr == 0x60 ){
		...
		// device[handle].callback = tmp_callback;
		// xSemaphoreGive(i2c_lock);
		// printf("6 I2CSTA\t\t: 0x%02X\n\r", pca9665_read_reg(0, I2CSTA));
		// pca9665_write_reg(handle, I2CCON, CON_AA | CON_ST);
		// printf("7 I2CSTA\t\t: 0x%02X\n\r", pca9665_read_reg(0, I2CSTA));
		// if (device[0].rx.frame != NULL) {
		// 	csp_buffer_free_isr(device[0].rx.frame);
		// 	device[0].rx.frame = NULL;
		// }		
	// }
	// printf("receive from here\n");
	frame->dest =  addr;
	...
	/* If the node is 0x60 or 0x61 (COM board) */
	...
	/* this is to test ADCS i2c issue, add another non sense code to let eps work fine */
	// if (addr == 87) {
	// 	frame->dest =  127;
	// 	i2c_send(handle, frame, 0) ;
	// 	device[handle].callback = tmp_callback;
