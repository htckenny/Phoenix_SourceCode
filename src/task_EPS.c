...
{
	uint8_t uchar_eps[12]; //augmented neccessary data
	...
	{
		//time = time + delay;
		//time = time + (stop_time - start_time)*(1000/configTICK_RATE_HZ);
		printf("\n %4d %s \n", i, time_unit);
		...
		{
			error = 0;
			memcpy(&uchar_eps[0], &rxbuf[6], 4); // vboost,vbatt
			//memcpy(&uchar_eps[2], &rxbuf[8], 2); //vbatt
			memcpy(&uchar_eps[4], &rxbuf[14], 6); //current in ,current into battery,current out
		...
		{
			if (abs((vbatt - rate[j])) >= 0.002 * rate[j] && (vbatt - rate[j]) != 0) //wherether does the circumstance change
				period = 0;
			else
				period++;
		...
		if (period > 3 && ( choose == 97 || choose == 99) )// trigger the warning signal for charge
		{
	...
	//ptr = (uint8_t*)malloc(numbyte * sizeof(uint8_t));
	//for (i = 0; i < numbyte; i++)
	//	*(ptr + i) = uchar_eps[i];
	//eps_write(ptr, choose);			//write into SD card
...
int eps_write(uint8_t *frameCont, int choose)
{
	...
	if (choose == 97)
	{
		...
		strcpy(fileName1, "BATTERY/batt_a.bin");
	}
	if (choose == 98)
	{
		//f_unlink("BATTERY/battery_b.bin");
		strcpy(fileName1, "BATTERY/batt_b.bin");
	}
	if (choose == 99)
	{
		//f_unlink("Battery/battery_c.bin");
		strcpy(fileName1, "BATTERY/batt_c.bin");
	}
	if (choose == 100)
	{
		//f_unlink("Battery/battery_d.bin");
		strcpy(fileName1, "BATTERY/batt_d.bin");
	}
	if (choose == 101)
	{
		//f_unlink("Battery/battery_e.bin");
		strcpy(fileName1, "BATTERY/batt_e.bin");
	}
	if (choose == 102)
	{
		//f_unlink("Battery/battery_f.bin");
		strcpy(fileName1, "BATTERY/batt_f.bin");
	}
	if (choose == 103)
	{
		//f_unlink("Battery/battery_g.bin");
		strcpy(fileName1, "BATTERY/batt_g.bin");
	}
	if (choose == 104)
	{
		//f_unlink("Battery/battery_h.bin");
		strcpy(fileName1, "BATTERY/batt_h.bin");
	}
	//}
	//cleanfile = 1;
	//printf("%s\n",fileName1 );
