...
void vTaskInit(void * pvParameters) {
	...
	spi_df_chip.baudrate = 5000000;	// This is only the initial baud rate, it will be increased by the driver
	...
	if (result != 0) {
		...
		uffs_Mount("/data/"); // new
		result = uffs_InitFileSystemObjects();
		if (result == U_SUCC) {
...
#ifdef ENABLE_NANOPOWER_SLAVE_CLIENT
